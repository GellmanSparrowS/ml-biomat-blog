...
如何选择？给出一个简单的判断标准：**如果磁盘空间充裕（余量 ≥ 10 GB）且你对命令行不熟悉，选 Anaconda；如果磁盘紧张、已有一定使用经验、或在服务器上工作，选 Miniconda**。两者日后都可以自由切换（删除旧安装后重新安装另一个），所以不存在"选错了"的风险，不必过度纠结。
...
判断某个包应该用 conda 还是 pip 安装，建议按以下顺序检查：第一步，在 conda-forge 频道搜索（`conda search -c conda-forge 包名`）；如果 conda-forge 有且版本足够新，用 conda 安装；如果没有或版本过旧，再用 pip 安装。conda-forge 是由开源社区维护的 conda 包仓库，覆盖极为广泛，包括 BioPython、pymatgen、FEniCSx、NGsolve 等专业科学计算包，大多数情况下 conda-forge 的版本更新速度并不慢于 PyPI。
